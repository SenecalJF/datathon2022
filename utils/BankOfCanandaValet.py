...
class ValetApi():
    ...
    def get_group_detail(self, group_name, format="csv"):
        ...
        res = requests.get(f"{self.BASE_URL}/groups/{group_name}/csv")
        return res.text
    ...
    def get_serie_observations(self, serie_name, start_date=None, format="csv"):
        ...
        res = requests.get(
            f"{self.BASE_URL}/observations/{serie_name}/csv{options}")
        return res.text
    ...
    def build_cvs_with_serie(self, serie_name, **kwargs):
        ...
        df.to_csv(f"data\dataset\CanBankValetApi\{col2_name}.csv")
        return df
